...
if old_hero in content:
    ...
    print("Could not find old hero block. Run git status to check state.")
...
# Define wrapping logic
def wrap_with_scroll(match, anim):
    return match.group(1) + match.group(2) + f'\n            <ScrollReveal animationClass="{anim}">\n              ' + match.group(3) + "\n            </ScrollReveal>\n          </section>"

# Features section
content = re.sub(r'(<section id="features" className="[^"]+">)(\s*)(<div className="max-w-7xl mx-auto">.*?)\s*</section>', lambda m: wrap_with_scroll(m, "translate-y-12"), content, flags=re.DOTALL)
# Trust section
